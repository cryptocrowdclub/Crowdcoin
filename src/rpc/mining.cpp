...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    uint64_t nMaxTries = 10000000000;
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
                "2. address      (string, required) The address to send the newly generated Crowdcoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Crowdcoin Core is downloading masternode winners...");
