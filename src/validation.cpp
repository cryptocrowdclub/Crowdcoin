...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0){
        return 80000000000*COIN;
    }
    CAmount nSubsidy = 3200 * COIN;

    CAmount nReduction = (nSubsidy*nPrevHeight*128)/100000;
    if ((nSubsidy-nReduction)<0) return 0;
    return nSubsidy-nReduction;
}
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    ret += (std::min(24, (nHeight-nMNPIBlock)/nMNPIPeriod)*blockValue) / 80;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // LP add burn rate
    if (pindex->pprev->nHeight > chainparams.GetConsensus().nBurnStartBlock){
        nFees -= (nFees * chainparams.GetConsensus().nBurnPercentage) / 1000;
    }
    // LP burn rate
