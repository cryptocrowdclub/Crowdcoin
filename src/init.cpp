...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "crowdcoin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N CRC anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/crowdcoinpay/crowdcoin>";
    const std::string URL_WEBSITE = "<https://crowdcoin.org>";
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ...
        ForceSetArg("-privatesendamount", i64tostr(MAX_PRIVATESEND_AMOUNT));
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/crowdcoinpay/crowdcoin/pull/1817 and https://github.com/crowdcoinpay/crowdcoin/pull/1743
    ...
    privateSendClient.nPrivateSendAmount = std::min(std::max((const long int)GetArg("-privatesendamount", DEFAULT_PRIVATESEND_AMOUNT), MIN_PRIVATESEND_AMOUNT), MAX_PRIVATESEND_AMOUNT);
#endif // ENABLE_WALLET
    ...
    // ********************************************************* Step 11d: start crowdcoin-ps-<smth> threads
